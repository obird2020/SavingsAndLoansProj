...
Public Class Banks
      ...
      If e.CommandName = "editCmd" Or e.CommandName = "dltCmd" Then
         ...
         If e.CommandName = "editCmd" Then
            ...
            If e.CommandName = "dltCmd" Then
               ...
               obj.delItm(Session("rowsID"), "banks")
               populateGrid()
            End If
   ...
   Protected Sub validateCode(ByVal source As Object, ByVal arguments As ServerValidateEventArgs)
      Using con As New MySqlConnection(ConfigurationManager.ConnectionStrings("microConn").ConnectionString)
         ...
         If Session("rowsID") = 0 Then
            cmd = New MySqlCommand("SELECT * FROM banks WHERE code=@code", con)
            cmd.Parameters.AddWithValue("@code", arguments.Value)
         Else
            If txtCode.Text.Trim = Session("code") Then
               cmd = New MySqlCommand("SELECT * FROM banks WHERE code='POLKJHGTFDERRYYUUJJFDESOLMGDS'", con)
            Else
               cmd = New MySqlCommand("SELECT * FROM banks WHERE code=@code", con)
               cmd.Parameters.AddWithValue("@code", arguments.Value)
            End If
   ...
   Protected Sub validatedescription(ByVal source As Object, ByVal arguments As ServerValidateEventArgs)
      Using con As New MySqlConnection(ConfigurationManager.ConnectionStrings("microConn").ConnectionString)
         Dim cmd As MySqlCommand
         If Session("rowsID") = 0 Then
            cmd = New MySqlCommand("SELECT * FROM banks WHERE description=@description", con)
            cmd.Parameters.AddWithValue("@description", arguments.Value)
         Else
            If txtDescription.Text.Trim = Session("description") Then
               cmd = New MySqlCommand("SELECT * FROM banks WHERE description='POLKJHGTFDERRYYUUJJFDESOLMGDS'", con)
            Else
               cmd = New MySqlCommand("SELECT * FROM banks WHERE description=@description", con)
               cmd.Parameters.AddWithValue("@description", arguments.Value)
            End If
   ...
   Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
      If Page.IsValid Then
         ...
         If Session("rowsID") = 0 Then
            config.addCommonSetup(txtCode.Text.Trim, txtDescription.Text.Trim, Math.Abs(CInt(chkInactive.Checked)), "banks", Session("ID"))
            lblmsg.Text = "Bank Saved"
         Else
            config.updateCommonSetup(txtCode.Text.Trim, txtDescription.Text.Trim, Math.Abs(CInt(chkInactive.Checked)), "banks", Session("rowsID"), Session("ID"))
            lblmsg.Text = "Bank Updated"
         End If
         txtDescription.Text = ""
         txtCode.Text = ""
         chkInactive.Checked = False
