...
Public Class Territories
   ...
   Protected Sub validateCode(ByVal source As Object, ByVal arguments As ServerValidateEventArgs)
      Using con As New MySqlConnection(ConfigurationManager.ConnectionStrings("microConn").ConnectionString)
         ...
         If Session("rowsID") = 0 Then
            cmd = New MySqlCommand("SELECT * FROM territories WHERE code=@code", con)
            cmd.Parameters.AddWithValue("@code", arguments.Value)
         Else
            If txtCode.Text.Trim = Session("code") Then
               cmd = New MySqlCommand("SELECT * FROM territories WHERE code='POLKJHGTFDERRYYUUJJFDESOLMGDS'", con)
            Else
               cmd = New MySqlCommand("SELECT * FROM territories WHERE code=@code", con)
               cmd.Parameters.AddWithValue("@code", arguments.Value)
            End If
   ...
   Protected Sub validatedescription(ByVal source As Object, ByVal arguments As ServerValidateEventArgs)
      Using con As New MySqlConnection(ConfigurationManager.ConnectionStrings("microConn").ConnectionString)
         Dim cmd As MySqlCommand
         If Session("rowsID") = 0 Then
            cmd = New MySqlCommand("SELECT * FROM territories WHERE description=@description", con)
            cmd.Parameters.AddWithValue("@description", arguments.Value)
         Else
            If txtDescription.Text.Trim = Session("description") Then
               cmd = New MySqlCommand("SELECT * FROM territories WHERE description='POLKJHGTFDERRYYUUJJFDESOLMGDS'", con)
            Else
               cmd = New MySqlCommand("SELECT * FROM territories WHERE description=@description", con)
               cmd.Parameters.AddWithValue("@description", arguments.Value)
            End If
   ...
   Protected Sub populateGrid()
      ...
      Using con As New MySqlConnection(ConfigurationManager.ConnectionStrings("microConn").ConnectionString)
         Dim cmd As MySqlCommand = New MySqlCommand("SELECT * from territories where ID > 0 order by description, code", con)
         con.Open()
         Dim reader As MySqlDataReader = cmd.ExecuteReader()
   ...
   Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
      If Page.IsValid Then
         ...
         If Session("rowsID") = 0 Then
            config.addCommonSetup(txtCode.Text.Trim, txtDescription.Text.Trim, Math.Abs(CInt(chkInactive.Checked)), "territories", Session("ID"))
            lblmsg.Text = "Territory Saved"
         Else
            config.updateCommonSetup(txtCode.Text.Trim, txtDescription.Text.Trim, Math.Abs(CInt(chkInactive.Checked)), "territories", Session("rowsID"), Session("ID"))
            lblmsg.Text = "Territory Updated"
         End If
         txtDescription.Text = ""
         txtCode.Text = ""
         chkInactive.Checked = False
