...
Public Class SecurityTypes
   ...
   Protected Sub validateCode(ByVal source As Object, ByVal arguments As ServerValidateEventArgs)
      Using con As New MySqlConnection(ConfigurationManager.ConnectionStrings("microConn").ConnectionString)
         ...
         If Session("rowsID") = 0 Then
            cmd = New MySqlCommand("SELECT * FROM loan_security_types WHERE code=@code", con)
            cmd.Parameters.AddWithValue("@code", arguments.Value)
         Else
            If txtCode.Text.Trim = Session("code") Then
               cmd = New MySqlCommand("SELECT * FROM loan_security_types WHERE code='POLKJHGTFDERRYYUUJJFDESOLMGDS'", con)
            Else
               cmd = New MySqlCommand("SELECT * FROM loan_security_types WHERE code=@code", con)
               cmd.Parameters.AddWithValue("@code", arguments.Value)
            End If
   ...
   Protected Sub validatedescription(ByVal source As Object, ByVal arguments As ServerValidateEventArgs)
      Using con As New MySqlConnection(ConfigurationManager.ConnectionStrings("microConn").ConnectionString)
         Dim cmd As MySqlCommand
         If Session("rowsID") = 0 Then
            cmd = New MySqlCommand("SELECT * FROM loan_security_types WHERE description=@description", con)
            cmd.Parameters.AddWithValue("@description", arguments.Value)
         Else
            If txtDescription.Text.Trim = Session("description") Then
               cmd = New MySqlCommand("SELECT * FROM loan_security_types WHERE description='POLKJHGTFDERRYYUUJJFDESOLMGDS'", con)
            Else
               cmd = New MySqlCommand("SELECT * FROM loan_security_types WHERE description=@description", con)
               cmd.Parameters.AddWithValue("@description", arguments.Value)
            End If
   ...
   Protected Sub populateGrid()
      ...
      Using con As New MySqlConnection(ConfigurationManager.ConnectionStrings("microConn").ConnectionString)
         Dim cmd As MySqlCommand = New MySqlCommand("SELECT * from loan_security_types where ID > 0 order by description, code", con)
         con.Open()
         Dim reader As MySqlDataReader = cmd.ExecuteReader()
   ...
   Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
      If Page.IsValid Then
         ...
         If Session("rowsID") = 0 Then
            config.addLoanSecurityType(txtCode.Text.Trim, txtDescription.Text.Trim, cboType.SelectedValue, Math.Abs(CInt(chkInactive.Checked)), Session("ID"))
            lblmsg.Text = "Security type Saved"
         Else
            config.updateLoanSecurityType(txtCode.Text.Trim, txtDescription.Text.Trim, cboType.SelectedValue, Math.Abs(CInt(chkInactive.Checked)), Session("rowsID"), Session("ID"))
